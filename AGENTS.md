...
- `core/teardown.py` — the `_teardown`/`remove()` phase engine (ticket #135): `_TeardownContext`, `build_context()`, `_target_is_absent()`, `_TEARDOWN_PHASES`, `run_teardown()`; `_phase_orphan_scan` — the all-platform, warn-only orphan-detection phase at index 6 (ticket #140). See "Teardown/remove changes" below before touching this path.
