...
- `core/state.py` — `StateStore` protocol + `WorktreeRecord` dataclass (`backing: "worktree" | "primary"`, ticket #84; `stop_detail: Optional[StopDetail]` — machine-readable reason for a `"stop_incomplete"` status, ticket #99; `stop_hook_outcome: Optional[StopHookOutcome]` — stop-hook + contract diagnostics, ticket #128)
