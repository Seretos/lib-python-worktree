...
class TestMatrixHistoricalTickets:
    ...
    def test_ticket_103_seretos_only_dirt_does_not_block_removal(self, tmp_path):
        """#103: the ONLY dirt is the benign .seretos/ convenience copy ->
        does not raise DirtyWorktreeError. Re-expressed for #154: the
        `.seretos/` exemption now lives entirely in the unconditional dirt
        gate's existing `_real_dirt_paths`/`_is_exempt_untracked` logic
        (item 16) -- there is no more git-driven `--force` retry to assert
        on, since `git worktree remove` is no longer part of the removal
        mechanism at all; the observable behaviour that survives is simply
        that the removal succeeds instead of raising."""
        manager = _make_manager(tmp_path)
        checkout = tmp_path / "wt-t103"
        checkout.mkdir()
        record = _make_record(
            "wt-t103", path=str(checkout), repo_root=str(tmp_path)
        )
        manager.state.add(record)
        mock_lifecycle = MagicMock()
        ...
        with patch(
            ...
            # Must not raise DirtyWorktreeError.
            manager._teardown(record, force=False, _lifecycle_module=mock_lifecycle)
    ...
    def test_ticket_126_teardown_steps_not_rerun_on_forced_retry(self, tmp_path):
        ...
        Re-expressed for #154 with a REAL checkout directory (the original
        row used a fake, nonexistent path, which under the new
        target_absent-gated dirt gate skips the dirt gate entirely and so
        could never legitimately raise DirtyWorktreeError at all -- a bug
        in the original row's premise once the removal mechanism changed,
        fixed here rather than left silently un-red)."""
        manager = _make_manager(tmp_path)
        checkout = tmp_path / "wt-t126"
        checkout.mkdir()
        record = _make_record(
            "wt-t126", path=str(checkout), repo_root=str(tmp_path)
        )
        manager.state.add(record)
        mock_lifecycle = MagicMock()
        ...
        assert record.teardown_ran is True
        assert checkout.exists(), "the dirt gate's undo must put the tree back"

        # Retry with force=True: the dirt gate is skipped entirely
        # (ctx.force), but teardown_ran==True must still gate the steps
        # from running again.
        with (
            patch(
                "lib_python_worktree.core.teardown._load_contract",
...
class TestMatrixRemovalMechanism:
    ...
    def test_locked_staged_dir_error_names_removing(self, tmp_path):
        ...
        with (
            ...
            # win32 rung 4 (robocopy) is a real subprocess, not
            # shutil.rmtree -- no-op it too so the residual genuinely
            # persists through every rung.
    ...
    def test_final_guard_on_a_surviving_original_keeps_the_v0312_message(
        ...
        assert excinfo.value.staged is False
        # kill_blocking_processes was never opted into (default False), so
        # ctx.kill_attempted stays False -- the kill_attempted=False
        # phrasing applies, exactly as v0.3.12's own WorktreeDirLockedError
        # already selects by kill_attempted. (Fixed during implementation:
        # this test originally asserted the kill_attempted=True phrasing,
        # which was wrong given the test never opts into a kill.)
        assert str(excinfo.value) == (
            f"worktree '{record.id}' directory is locked by another process. "
            f"Pass kill_blocking_processes=True to kill the blocking "
            f"process(es) and retry."
        )
